...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "inspect_code": "Read-only self-inspection of Atlas/Odysseus\' REAL host repository. Use to understand your own implementation, architecture, current branch/commit, source files, symbols, and uncommitted diffs. Actions: status, tree, search, read, diff. This tool cannot write, run arbitrary commands, or read blocked secrets/runtime data; prefer it over bash/read_file when the user asks about your own code.",
    ...
    "proxmox": "Read-only Proxmox VE homelab monitoring. Check node CPU/RAM and health, VMs, LXCs, rich guest details, CPU/RAM/disk/network RRD history, guest status/config, storage capacity, recent tasks/errors, and diagnostics. Never performs power or destructive actions.",
    "kucoin_bot": "Read-only KuCoin grid-bot telemetry. Check bot state, persisted-anchor portfolio value, P&L/return, peak value, drawdown, fee/fill counts, and day/week/month telemetry history. Uses only the bot's persisted anchor/accounting state: no live market price, no KuCoin private API access, and no ability to place/cancel trades.",
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Covers calendar, gallery, library/documents, notes, tasks, settings, research, compare, cookbook GPUs/state — allowed UI buttons hit /api/* endpoints and you can hit them too. Native /api/memory* and /api/codex/memory* routes are deliberately blocked: all agent memory operations must use manage_memory so the configured memory backend remains authoritative. Sensitive auth/user/admin/shell paths and host-control Cookbook mutation routes are blocked; do NOT use app_api for shell commands, package installs, engine rebuilds, or PID signalling. Use named command tooling for shell commands. action='endpoints' with filter=<keyword> lists available endpoints. action='call' takes method+path+body. Hits same routes the UI uses — auth flows free. NOTE: themes are NOT an API endpoint — use the ui_control tool (create_theme / set_theme), not app_api. SESSIONS/CHATS: do NOT use app_api for these — GET /api/sessions returns EMPTY for tool calls (it's owner-filtered and tool calls authenticate as a different identity). EMAIL ACCOUNTS: do NOT use /api/email/accounts via app_api; use list_email_accounts, list_emails, and read_email instead. To list/rename/archive/delete/fork chats use the list_sessions and manage_session tools instead.",
